...
logdir="${xav}/ssl_epilepsy/data/new_patient_pyg"
...
echo "Submitting a job for the model with the given sample_ratio and time..."
sbatch <<EOT
#!/bin/bash
...
python patch.py "${patient_dir}" "${patient}" "${logdir}" "${model}" "${sample_ratio}" || { echo "Python script execution failed"; exit 1; }
